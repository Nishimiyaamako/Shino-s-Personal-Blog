...
- 决策：用户感知入口采用单域名：`https://<domain>`，后台入口路径 `/admin/login`。
- 决策：前台与后台路径共享同一后端服务，API 契约不分裂。
- 决策：本机调试主推统一域名入口（local reverse proxy），隐藏端口心智负担。
- 决策：开发期 API 代理保持 Vite 现状（`/api`、`/uploads` -> `3001`）。
- 决策：生产部署默认采用"1Panel 静态站点 + PM2 后端进程常驻 + 单域名入口"。
- 决策：后端不再使用 Docker 容器部署，改为 PM2 进程常驻（`ecosystem.config.js` 管理）。
...
- 风险：PM2 进程崩溃后若未配置自动重启，会导致服务中断。
  - 防护：`ecosystem.config.js` 已配置 `autorestart: true` 与 `max_restarts: 10`，并配合 `pm2 startup` 实现开机自启。

## 5) 当前任务与下一步（预算 ≤ 35 行）

- 当前任务：取消 Docker 部署，切换为 PM2 进程常驻方案，完成文档与配置同步。
- 下一步 1：在服务器上首次部署并验证 PM2 配置（`ecosystem.config.js`）。
- 下一步 2：根据需求继续迭代前台或后台功能。
- 下一步 3：保持 MEMORY.md 与 STOP_HOOKS.md 的时效性。

## 6) 最近更新记录（预算 ≤ 10 行）

- 2026-04-26：取消 Docker 部署，切换为 PM2 进程常驻，生成 ecosystem.config.js，更新全部部署文档。
...
- ~~决策：本机 Docker 反代默认采用 `--network host`，避免前端绑定 `127.0.0.1` 时 bridge 模式返回 `502`。~~（已废弃，2026-04-26 取消 Docker 部署）
- ~~风险：服务器到 Docker Hub 出口不可达时，后端镜像构建会超时，导致上线步骤中断。~~（已废弃，2026-04-26 取消 Docker 部署）
