...
- 决策：动效配置采用 `motionProfile: calm|vivid`，默认 `calm`，通过根节点 `data-motion-profile` 切换。
...
- 当前任务：统一文章卡片与路由动效收尾验收，并启用“每次修改后自动 memory sync”流程。
- 下一步 1：人工验收 `/`、`/posts`、`/tags`、`/tags/:tag` 的卡片顺序浮现与路由轻转场。
- 下一步 2：根据观感在 `frontend/src/config/site.ts` 切换 `motionProfile`（`calm`/`vivid`）并定稿。
- 下一步 3：部署前继续沿用 1Panel 发布清单，保持 Nginx SPA 回退与备案核查。
...
- 2026-03-21：完成首页/文章页/标签页/标签详情/标签面板的统一卡片动效编排与路由轻转场重构。
