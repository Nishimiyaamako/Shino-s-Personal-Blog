...
- 当前任务：将“前端自动技能路由基线”同步到当前项目文档与 bootstrap 模板，并完成继承验收。
- 下一步 1：在临时目录运行 bootstrap 生成文档，确认默认链规则可继承到新项目。
- 下一步 2：在下一次 frontend 实任务中验证 `frontend-preflight-skill-stack` 的触发与优先级行为。
- 下一步 3：按真实任务反馈决定是否需要为特定场景引入 `optimize/normalize` 条件化链路。
